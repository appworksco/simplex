...
$partnersItems = $webFacade->fetchHomePartners()->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_GET["is_updated"])) {
    $homePartnersId = $_GET["is_updated"];
}
...
if (isset($_POST["add_home_partners"])) {
    $name = $_POST["name"];
    ...
    if (empty($name)) {
        ...
        $webFacade = new WebFacade;
        $webFacade->addHomePartners($name, $file_path, $caption);
        
        // I-move ang uploaded file sa tamang directory
        move_uploaded_file($file_tmp, $file_path);

        header("Location: home-partners.php");
        exit();
    }
}

if(isset($_POST['update_home_partners'])) {
    $name = $_POST['name'];
    $caption = $_POST['caption'];
    ...
    $updateHomePartners = $webFacade->updateHomePartners($name, $file_path, $caption, $id);
}
    ...
    <div class="card w-100" style="background-color: #BFC9C7;">
        ...
        <div>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="card-title fw-bold my-2 text-center">Manage Home Partners</h5>
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addHomePartners">Add</button>
            </div>
            <div class="py-2">
                <?php include('../../errors.php') ?>
            </div>
            <div class="row row-cols-1 row-cols-md-4 g-4">
                <?php foreach ($partnersItems as $item): ?>
                <div class="col">
                    <div class="about-item border border-2 p-3" style="background-color: #03382F;">
                        ...
                        <div class="action-buttons">
                            <a href="home-partners?is_updated=<?= $item["id"] ?>" class="btn btn-sm btn-info me-2">Update</a>
                            <a href="delete-home-partners?home_partners_id=<?= $item["id"] ?>" class="btn btn-sm btn-danger me-2" onclick="return confirm('Are you sure you want to delete this?');">Delete</a>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
...
<?php include realpath(__DIR__ . '/../../includes/modals/web/add-home-partners.php') ?>
<?php include realpath(__DIR__ . '/../../includes/modals/web/update-home-partners.php') ?>
