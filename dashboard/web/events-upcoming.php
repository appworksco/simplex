...
$upcomingItems = $webFacade->fetchEventsUpcoming()->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_GET["is_updated"])) {
    $eventsUpcomingId = $_GET["is_updated"];
}
...
if (isset($_POST["add_events_upcoming"])) {
    $name = $_POST["name"];
    ...
    if (empty($name)) {
        ...
        $webFacade = new WebFacade;
        $webFacade->addEventsUpcoming($name, $file_path, $caption);
        ...
        header("Location: events-upcoming.php");
        exit();
    }
}

if(isset($_POST['update_events_upcoming'])) {
    $name = $_POST['name'];
    $caption = $_POST['caption'];
    ...
    $updateEventsUpcoming = $webFacade->updateEventsUpcoming($name, $file_path, $caption, $id);
}
        ...
        <div class="card w-100 mt-3" style="background-color: #EEE38A;">
            <div class="card-body p-4">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="card-title fw-bold my-2 text-center">Manage Events Upcoming</h5>
                    <button type="button" class="btn btn-lg btn-primary" data-bs-toggle="modal" data-bs-target="#addEventsUpcoming">Add</button>
                </div>
                <div class="py-2">
                        ...
                        <div class="col-md-6">
                            <div class="about-item border border-2 p-3" style="background-color: #03382F;">
                                ...
                                <div class="action-buttons">
                                    <a href="events-upcoming?is_updated=<?= $item["id"] ?>" class="btn btn-sm btn-info me-2">Update</a>
                                    <a href="delete-events-upcoming?events_upcoming_id=<?= $item["id"] ?>" class="btn btn-sm btn-danger me-2" onclick="return confirm('Are you sure you want to delete this?');">Delete</a>
                                </div>
                            </div>
                        </div>
                        <?php $count++; ?>
                        <?php if ($count % 2 == 0 || $count == count($upcomingItems)): ?>
                            </div>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </div>
...
<?php include realpath(__DIR__ . '/../../includes/modals/web/add-events-upcoming.php') ?>
<?php include realpath(__DIR__ . '/../../includes/modals/web/update-events-upcoming.php') ?>
    ...
    if ($eventsUpcomingId) {
        echo '<script type="text/javascript">
                $(document).ready(function(){
                    $("#updateEventsUpcoming").modal("show");
                });
            </script>';
    } else {
    }
