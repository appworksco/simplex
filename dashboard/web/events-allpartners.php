...
$allpartnersItems = $webFacade->fetchEventsAllpartners()->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_GET["is_updated"])) {
    $eventsAllpartnersId = $_GET["is_updated"];
}
...
if (isset($_POST["add_events_allpartners"])) {
    $name = $_POST["name"];
    ...
    if (empty($name)) {
        ...
        $webFacade = new WebFacade;
        $webFacade->addEventsAllpartners($name, $file_path, $caption);
        ...
        header("Location: events-allpartners.php");
        exit();
    }
}

if(isset($_POST['update_events_allpartners'])) {
    $name = $_POST['name'];
    $caption = $_POST['caption'];
    ...
    $updateEventsAllpartners = $webFacade->updateEventsAllpartners($name, $file_path, $caption, $id);
}
    ...
    <div class="card w-100" style="background-color: #BFC9C7;">
        ...
        <div>
            ...
            <div class="d-flex justify-content-between">
                <h5 class="card-title fw-semibold my-2">Manage Events All Partners</h5>
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addEventsAllpartners">Add</button>
            </div>
            <div class="py-2">
                <?php include('../../errors.php') ?>
            </div>
            <div class="row row-cols-1 row-cols-md-4 g-4">
                <?php foreach ($allpartnersItems as $item): ?>
                <div class="col-md-3">
                    <div class="about-item border border-2 p-3" style="background-color: #03382F;">
                        ...
                        <div class="action-buttons">
                            <a href="events-allpartners?is_updated=<?= $item["id"] ?>" class="btn btn-sm btn-info me-2">Update</a>
                            <a href="delete-events-allpartners?events_allpartners_id=<?= $item["id"] ?>" class="btn btn-sm btn-danger me-2" onclick="return confirm('Are you sure you want to delete this?');">Delete</a>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
...
<?php include realpath(__DIR__ . '/../../includes/modals/web/add-events-allpartners.php') ?>
<?php include realpath(__DIR__ . '/../../includes/modals/web/update-events-allpartners.php') ?>
<?php include realpath(__DIR__ . '/../../includes/layout/web-footer.php') ?>

<?php
if (isset($_GET["is_updated"])) {
    $eventsAllpartnersId = $_GET["is_updated"];
    if ($eventsAllpartnersId) {
        echo '<script type="text/javascript">
                $(document).ready(function(){
                    $("#updateEventsAllpartners").modal("show");
                });
            </script>';
    } else {
    }
